...
class AddAdministratorUser extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      name: "",
  ...
  componentDidMount() {
    axios.get(`http://localhost:5050/country/allcountry`).then((res) => {
  ...
  handleSubmit(event) {
    ...
    if (this.validator.allValid()) {
      ...
      console.log(menu);
      axios
        .post(`http://localhost:5050/administrator/administratorusers`, menu)
        .then((res) => {
          console.log(res);
          console.log(res.data);
        });

      this.props.history.push("/administratoruser");
  ...
  setCityName(e) {
    ...
    axios
      .get(`http://localhost:5050/city/cityvalues/${selectedCountryName}`)
      .then((res) => {
